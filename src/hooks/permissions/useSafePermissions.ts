...
type UseSafePermissionsReturnType = {
  ...
  updatePermission: (
    origin: string,
    changeset: SafePermissionChangeSet,
  ) => void;
...
const useSafePermissions = (): UseSafePermissionsReturnType => {
  const [permissions = {}, setPermissions] =
    useLocalStorage<SafePermissions>(SAFE_PERMISSIONS);

  const [permissionsRequest, setPermissionsRequest] = useState<
    SafePermissionsRequest | undefined
  >();
            ...
            if (change.selected) {
              permission.caveats =
                permission.caveats?.filter(
                  (caveat) => caveat.type !== USER_RESTRICTED,
                ) || [];
  ...
  const hasPermission = useCallback(
    (origin: string, permission: Methods) => {
      return permissions[trimTrailingSlash(origin)]?.some(
        (p) =>
          p.parentCapability === permission && !isUserRestricted(p.caveats),
      );
    },
    [permissions],
  );
  ...
  const confirmPermissionRequest = useCallback(
    (result: PermissionStatus) => {
      ...
      const updatedPermissionsByOrigin = [
        ...(permissions[permissionsRequest.origin] || []),
      ];
  ...
  const isUserRestricted = (caveats?: any[]) =>
    !!caveats?.some(
      (caveat) => caveat.type === USER_RESTRICTED && caveat.value === true,
    );
