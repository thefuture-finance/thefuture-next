...
type BrowserPermissionChangeSet = {
  feature: AllowedFeatures;
  selected: boolean;
}[];
...
type UseBrowserPermissionsReturnType = {
  ...
  updatePermission: (
    origin: string,
    changeset: BrowserPermissionChangeSet,
  ) => void;
...
const useBrowserPermissions = (): UseBrowserPermissionsReturnType => {
      ...
      setPermissions({
        ...
        [appUrl]: permissions[appUrl].map((p) => {
          const change = changeset.find(
            (change) => change.feature === p.feature,
          );

          if (change) {
            p.status = change.selected
  ...
  const addPermissions = useCallback(
    (origin: string, selectedPermissions: BrowserPermission[]) => {
      setPermissions({
        ...permissions,
        [trimTrailingSlash(origin)]: selectedPermissions,
      });
    },
    [permissions, setPermissions],
  );
